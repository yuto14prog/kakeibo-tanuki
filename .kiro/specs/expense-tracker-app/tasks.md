...
- [ ] 3. データモデルとバリデーションの実装
- [ ] 3.1 コアデータモデルインターフェースと型の作成
  - Card、Category、Expense構造体の定義
  - バリデーション関数の実装
  - データ整合性チェック機能の実装
  - _要件: 1.3, 4.2, 7.1_

- [ ] 3.2 Cardモデルとバリデーションの実装
  - Card構造体とGORMタグの実装
  - カード名バリデーションの単体テスト作成
  - カード作成・更新のバリデーション機能
  - _要件: 1.4_

- [ ] 3.3 Categoryモデルとデフォルトカテゴリの実装
  - Category構造体の実装
  - デフォルトカテゴリ（食費、交通費等）のシードデータ作成
  - カテゴリ名の一意性制約とバリデーション
  - _要件: 4.1_

- [ ] 3.4 Expenseモデルと関連性の実装
...
- [ ] 12.1 バックエンド統合テストの作成
  - API エンドポイントの統合テスト作成
  - データベース操作を含むテストの実装
  - テストデータベースの設定とクリーンアップ
  - _要件: 全般的な機能要件_

- [ ] 12.2 フロントエンドE2Eテストの作成
  - Playwright を使用したE2Eテストの設定
  - 主要ユーザーフローのテスト作成
  - 支出登録から一覧表示までの一連のテスト
  - _要件: 全般的なユーザーフロー要件_
